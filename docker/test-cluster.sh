...
[ "$login_code" = 302 ] || { echo "alice login failed: HTTP $login_code" >&2; exit 1; }
...
i=0
until [ "$(curl -sS -b "$cookie_jar" -o /dev/null -w '%{http_code}' "http://127.0.0.1:$test_port/" || true)" = 200 ]; do
  i=$((i + 1))
...
registered_login_code=$(curl -sS -b "$registered_jar" -c "$registered_jar" -o /dev/null -w '%{http_code}' \
  -X POST "http://127.0.0.1:$test_port/login" \
  --data-urlencode "csrf=$csrf" \
